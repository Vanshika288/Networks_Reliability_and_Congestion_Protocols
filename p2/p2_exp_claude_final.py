...
def run_trial(output_handle, bw=100, loss=0, delay_c2_ms=5, udp_off_mean=None, iteration=0, buffer_size=420):
    ...
    server_py = "p2_server_claude_final.py"
    ...
    client_py = "p2_client_claude_final.py"

    # Define log files for clients
    c1_log = f"{logs_dir}/c1_client_bw{bw}_loss{loss}_delay{delay_c2_ms}_iter{iteration}.log"
    c2_log = f"{logs_dir}/c2_client_bw{bw}_loss{loss}_delay{delay_c2_ms}_iter{iteration}.log"
    ...
    c1_pid_raw = c1.cmd(f"bash -c '{c1_start_cmd} > {c1_log} 2>&1 & echo $!'").strip()
    c2_pid_raw = c2.cmd(f"bash -c '{c2_start_cmd} > {c2_log} 2>&1 & echo $!'").strip()
    if c1_pid_raw:
        c1_pid = c1_pid_raw.split()[0]
        print(f"started client 1 with PID: {c1_pid} (log: {c1_log})")
    else:
    ...
    if c2_pid_raw:
        ...
        print(f"started client 2 with PID: {c2_pid} (log: {c2_log})")
    else:
...
def run_trial_with_udp(output_handle, bw=100, loss=0, delay_c2_ms=5, udp_off_mean=1.0, iteration=0, buffer_size=420):
    ...
    # Define log files for TCP servers
    ...
    s3_log = f"{logs_dir}/s3_udp_server_bw{bw}_udpmean{udp_off_mean}_iter{iteration}.log"
    s3_pid_raw = s3.cmd(f"bash -c 'python3 udp_server.py {s3.IP()} {UDP_SERVER_PORT} {udp_off_mean} > {s3_log} 2>&1 & echo $!'").strip()
    s3_pid = s3_pid_raw.split()[0] if s3_pid_raw else None
    print(f"started UDP server s3 pid: {s3_pid} (log: {s3_log})")
    ...
    # Define log files for TCP clients
    c1_log = f"{logs_dir}/c1_client_udp_bw{bw}_loss{loss}_udpmean{udp_off_mean}_iter{iteration}.log"
    c2_log = f"{logs_dir}/c2_client_udp_bw{bw}_loss{loss}_udpmean{udp_off_mean}_iter{iteration}.log"
    ...
    if c1_pid_raw:
        c1_pid = c1_pid_raw.split()[0]
        print(f"started TCP client 1 with PID: {c1_pid} (log: {c1_log})")
    else:
        print(" warning: no pid returned for client1 start; will use pgrep to detect completion")
        c1_pid = None
    ...
    if c2_pid_raw:
        c2_pid = c2_pid_raw.split()[0]
        print(f"started TCP client 2 with PID: {c2_pid} (log: {c2_log})")
    else:
        print(" warning: no pid returned for client2 start; will use pgrep to detect completion")
        c2_pid = None

    # Start UDP client on c3
    c3_log = f"{logs_dir}/c3_udp_client_bw{bw}_udpmean{udp_off_mean}_iter{iteration}.log"
    c3_start_cmd = f"python3 udp_client.py {s3.IP()} {UDP_SERVER_PORT}"
    c3_pid_raw = c3.cmd(f"bash -c '{c3_start_cmd} > {c3_log} 2>&1 & echo $!'").strip()
    c3_pid = c3_pid_raw.split()[0] if c3_pid_raw else None
    print(f"started UDP client c3 with PID: {c3_pid} (log: {c3_log})")
    ...
    # --- Kill any remaining server processes if still running ---
    print("stopping servers (if still active)")
    s1.cmd("pkill -f p2_server_claude_final.py || true")
    s2.cmd("pkill -f p2_server_claude_final.py || true")
