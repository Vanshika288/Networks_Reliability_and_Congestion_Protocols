...
INITIAL_SSTHRESH_MSS = 64        # Initial slow start threshold (in MSS units)
MIN_CWND_MSS = 1                 # Minimum congestion window
MAX_CWND_MSS = 1000              # Maximum congestion window (safety limit)
...
class CubicCongestionControl:
    ...
    def __init__(self):
        ...
        self.ack_cnt = 0.0            # Count of ACKs in current RTT (for TCP-friendliness)
    ...
    def _cubic_update(self, current_time):
        ...
        if target > self.cwnd:
            # CUBIC is above current cwnd, increase
            cnt = self.cwnd / (target - self.cwnd)
        else:
            # CUBIC is below current cwnd (shouldn't happen often)
            cnt = 100.0 * self.cwnd
        
        # === TCP-FRIENDLINESS CHECK ===
        if CUBIC_TCP_FRIENDLINESS:
            # Estimate what TCP Reno would do (AIMD: +1 MSS per RTT)
            self.tcp_cwnd += (3.0 * CUBIC_BETA / (2.0 - CUBIC_BETA)) * (self.ack_cnt / self.cwnd)
            self.ack_cnt = 0.0
        ...
        # Increment cwnd by approximately 1 MSS per RTT
        delta = 1.0 / cnt
        self.cwnd += delta
...
def run_server(server_ip, server_port):
    ...
    while base_seq < file_size:
        with state_lock:
            ...
            # --- 4b. Send New Packets (PART 2: controlled by CUBIC cwnd) ---
            try_send_next_packets(sock)
            
            # Periodic state logging
            if current_time - last_state_print > 1.0:
                print(cubic_cc.get_state_str())
