...
EOF_MSG = b'EOF'
# --- PART 2 CHANGE: OUTFILE is now dynamic ---
...
def make_ack_packet(cum_ack, ts_echo, sack_block):
    ...
    sack_block: A tuple (start, ooo_bitmap) for the custom SACK.
    """
...
def get_sack_range(buffer, base_seq):
    """
    Return (start, bitmap) of received blocks above base_seq.
    'start' is the sequence number of the first OOO packet.
    'bitmap' is a 32-bit integer where the N-th bit is 1 if
    (start + N*DATA_LEN) has been received.
    """
    ...
    for seq in received:
        if seq > base_seq:
            if sack_start is None:
                sack_start = seq
                ooo_acks = 1 # The 0-th bit is for sack_start itself
            else:
                diff = (seq - sack_start) / DATA_LEN
                if 0 <= diff < 32: # Check if it fits in our 32-bit map
                    ooo_acks |= (1 << int(diff))
...
def run_client(server_ip, server_port, pref_filename): # --- PART 2 CHANGE: Added pref_filename ---
    """Main client logic."""
    global expected_seq
    
    # --- PART 2 CHANGE: Set output file name based on prefix ---
    ...
    try:
        with open(OUTFILE, 'wb') as f_out: # Uses the new OUTFILE variable
...
if __name__ == "__main__":
    # --- PART 2 CHANGE: Updated argument parsing ---
    if len(sys.argv) != 4:
