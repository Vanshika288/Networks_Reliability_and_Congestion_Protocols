...
HEADER_LEN = 20
PACKET_FORMAT = '!IIIII' # 4+4+4+4+4 = 20 bytes
...
class RTOEstimator:
    def __init__(self, alpha=0.125, beta=0.25, initial_rto=1.0, min_rto=0.01, max_rto=60.0):
        self.alpha = alpha
        self.beta = beta
        self.srtt = 0.0
        self.rttvar = 0.0
...
def make_packet(seq, data, timestamp_ms):
    ...
    header = struct.pack(PACKET_FORMAT, seq, timestamp_ms, 0, 0, 0)
    return header + data
...
def process_ack(ack_packet):
    ...
    try:
        ...
        cum_ack, ts_echo, sack_start, ooo_acks1, ooo_acks2 = struct.unpack(PACKET_FORMAT, ack_packet)
    except struct.error:
    ...
    with state_lock:
        ...
        if ooo_acks1 != 0:
            ...
            for seq in sack_keys:
                ...
                send_time_ms = int(send_time * 1000) & 0xFFFFFFFF
                sample_rtt_ms = (current_time_ms - send_time_ms) & 0xFFFFFFFF
        ...
        if cum_ack > base_seq:
            ...
            for seq in acked_keys:
                _packet, send_time, retrans_count = in_flight_packets.pop(seq)
                current_time_ms = int(time.time() * 1000) & 0xFFFFFFFF
                send_time_ms = int(send_time * 1000) & 0xFFFFFFFF
                sample_rtt_ms = (current_time_ms - send_time_ms) & 0xFFFFFFFF
                sample_rtt = sample_rtt_ms / 1000.0
